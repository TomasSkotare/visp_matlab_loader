...
class TestCaseRunner():
    ...
    def __init__(self, project: MatlabProject, verbose=True) -> None:
        ...
        for test in self.project.test_case_files:
            ...
            function_name, input_list, output_list = TestCaseRunner.convert_from_test_dict(test_json.read())
    ...
    def convert_from_test_dict(data):
        ...
        output_keys = sorted([k for k in data.keys() if 'output' in k], key=lambda x: int(x.split('_')[1]))
    ...
    def create_test_data(self, function_name, input_list, output_count):
        ...
        exit_code, matlab_output, return_value = script_executer.execute_script(function_name, output_count, *input_list)
        return exit_code, matlab_output, self.__class__.convert_to_test_dict(function_name, input_list, return_value.values())
...
def main():
    ...
    for project in tcr.project_finder.compiled_projects:
        ...
        for test_case in project.test_case_files:
            ...
            function_name, input_list, output_list = TestCaseRunner.convert_from_test_dict(data)
            ...
            exit_code, matlab_output, return_value = script_executer.execute_script(function_name, len(output_list), *input_list)
    ...
    exit_code, matlab_output, complete_test_dictionary = tcr.create_test_data('get_next_thousand', 'getnextthousand', [1], 1)
