...
def find_matlab_compiled(directory=os.path.dirname(os.path.realpath(__file__)), verbose=False):
    while directory != '/':
...
def find_and_import_package(package_name, directory=os.path.dirname(os.path.realpath(__file__))):
...
# Create the parser
parser = argparse.ArgumentParser(description='Voice analysis toolbox, with default values available for settings.')
...
parser.add_argument('--start_time', default=np.double(0), type=np.double, help='Start time')
...
parser.add_argument('--dfa_scaling', default=[float(x) for x in range(50,210,20)], type=int_list, help='DFA scaling values')
...
parser.add_argument('filename', type=str, help='Input filename')  # New filename argument
...
matlab_format['dfa_scaling'] = ensure_column_vector(matlab_format['dfa_scaling'])
