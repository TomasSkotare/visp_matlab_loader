...
# with TempLogLevel('visp_matlab_loader.execute.compiled_project_executor', logging.DEBUG):
#     result = f.execute(float(1000))
# import numpy as np
# assert np.all([x == y for x,y in zip(result.outputs['nextNumbers'], range(1001, 2000))]) # We check the values returned
...
with TempLogLevel('visp_matlab_loader.execute.compiled_project_executor', logging.DEBUG):
    result = gnt.getnextthreelists(inputNumber1=1, inputNumber2=2, inputNumber3=3, count=5)
assert np.all(result.outputs['list1'] == np.arange(2,7,dtype=np.uint8))
assert np.all(result.outputs['list2'] == np.arange(3,8,dtype=np.uint8))
assert np.all(result.outputs['list3'] == np.arange(4,9,dtype=np.uint8))
result.verify_serialization()
print('Serialization passed!')
...
with TempLogLevel('visp_matlab_loader.execute.compiled_project_executor', logging.DEBUG):
    result = gnt.getnextfivetypes(inputVal=10)
...
with TempLogLevel('visp_matlab_loader.execute.compiled_project_executor', logging.DEBUG):
    result = vat.voice_analysis_modified('/home/tomas/Projects/visp_matlab_loader/reference/audio_reference/Lx_ch1.wav',
                                                        start_time=20,
                                                        end_time=21)
